...
const v = os.join_path(vroot, 'v')
const testdata_folder = os.join_path(vroot, 'vlib', 'v', 'gen', 'llvm', 'testdata')
...
fn test_out_files() {
	...
	for out_path in paths {
		...
		pexe := os.join_path(output_path, '${basename}${ext}')
		//
		file_options := get_file_options(path)
		// TODO: remove the `-no-builtin` flag after builtin functions are implemented
		alloptions := '-b llvm -o ${os.quoted_path(pexe)} ${file_options.vflags}'
		println('alloptions: ${alloptions}')
		label := mj('villmo', file_options.vflags, 'run', relpath) + ' == ${mm(out_relpath)} '
		//
		sw_compile := time.new_stopwatch()
		println('executing: ${os.quoted_path(v)} ${alloptions} ${os.quoted_path(path)}')
		compilation := os.execute('${os.quoted_path(v)} ${alloptions} ${os.quoted_path(path)}')
...
// 		cmd := '${os.quoted_path(villmo)} ${alloptions} ${os.quoted_path(path)}'
// 		sw_compile := time.new_stopwatch()
